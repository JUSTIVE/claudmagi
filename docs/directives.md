...
- 오렌지 테마가 아닌 테마에서는 idle 칩을 주황 바탕 + 검은 라벨로. (#24)
- 선은 두 줄씩 짝지어 짝 사이 간격을 훨씬 넓힌다(짝 안 13, 짝 사이 43). 세션 칩은 짝의 첫 줄, 대각선 위에만 놓여 이웃과 겹치지 않는다(수직 40px). (#14, #29)
- 사용자 입력이 필요하거나 턴이 끝나면 그 칩의 선이 끊어진다(케이블 파형 → 홈 파인 소켓 → 분리된 칩, 앰버 색). (#5)
- 서브에이전트(Agent 도구)는 부모 세션 칩과 같은 선 위에 더 작은 칩으로 매달린다. 실행 중이면 연결, 끝나면 분리됐다가 45초 뒤 사라진다. 부모의 좌우로 번갈아 놓되 화면 밖으로 나갈 쪽은 피한다. (#12, #13, #22)
- 칩은 화면 왼쪽 20% 안쪽에 놓이지 않는다. 그 자리가 되면 다음 스트라이프의 대각선(오른쪽)을 쓴다. (#27, #30)
...
| # | 날짜 | 지시 | 상태 | 반영 위치 |
|---|------|------|------|-----------|
| 31 | 2026-09-10 | 두 줄씩 붙어 있는 짝의 두 줄 모두에 노드를 스폰. 노드끼리 겹치지 않게 간격은 더 벌리기 | ✅ | `src/render/scene.rs` (`chip_lane` = 모든 레인, GAP 38 / PAIR_GAP 24) |
| 32 | 2026-09-10 | 다크 테마에서는 동작 중인 노드의 배경색을 흰색으로 | ✅ | `src/theme.rs` (`DARK.chip`, `DARK.text_on`) |
| 33 | 2026-09-10 | 두 개씩 있는 노드(짝)의 간격을 좀 더 좁히기 | ✅ | `src/render/scene.rs` (GAP 33, 세션 칩 높이 20) |
| 34 | 2026-09-10 | 짝 안 간격을 28로 | ✅ | `src/render/scene.rs` (GAP 28, 세션 칩 17 / 서브 칩 14) |
