...
| 78 | 2026-09-23 | cmdk에서의 항목 디자인을 화면 내의 칩과 동일한 디자인으로 하는 건 어때 | ✅ | `src/ui/palette.rs`: 닷 + 평문 라벨을 버리고 행이 노드 자체를 그린다. `Style::dot()` → `Style::body()` 가 `scene` 과 같은 표에서 `(채움, 테두리, 글자)` 을 준다 — 세션은 `chip`/`chip_needs`/`chip_idle` 에 각각의 라벨색(보드가 소켓에서 빠질수록 저 색으로 lerp 하는 양 끝), PR·Linear 는 look/status 표, `Style::Pr(look, running)` 이라 CI 중이면 `busy` 테두리를 덮는다(#62). 치수도 보드를 따라 세션 22 / 커넥터 17, 라벨 12/10px. 테스트: 채움 = 더 진행됨(#58), CI 테두리가 몸통을 대체하지 않음, phase 3개가 테마마다 안 겹침, 라벨이 제 몸통과 같은 색이 되지 않음 |
| 79 | 2026-09-23 | 다 고만고만하게 생겨서 구별이 어렵네 · 칩 안쪽 왼쪽에 깃허브·클로드코드·리니어 로고 SVG · cmdk 칩 크기 통일 | ✅ | `assets/logos/`(simple-icons 15.22.0, 아이콘 파일은 CC0 · 마크 자체는 각 사의 상표) + `src/logos.rs` 의 `AssetSource` 가 `include_bytes!` 로 서빙, `main.rs` 에서 `with_assets`. 처음엔 기억으로 그린 근사 기호를 넣었다가 — 공식 마크를 정확히 재현할 수 없으니 — 인터넷에서 받아도 된다는 답을 받고 진품으로 교체했다. gpui 는 SVG 를 알파 마스크로 쓰므로 색은 칩 라벨색을 따른다. 칩은 150×20 하나로 통일(종류별 높이 차이 제거). 렉스트 검사만으로는 렌더 가능 여부를 모르므로 dev-dependency 로 `usvg` 를 물려 gpui 와 같은 파서로 파싱되고 제 박스를 채우는지를 테스트한다 — 못 읽는 파일은 빈 칩으로 나타나 보드 버그처럼 보인다 |
| 80 | 2026-09-23 | 기본 화면 칩에도 아이콘 추가해줄수 있니 | ✅ | 보드 칩은 gpui 엘리먼트가 아니라 자체 `Shape` 로 그리고 `--svg` 도 같은 길을 타므로, 오버레이 대신 마크를 셔이프로 내렸다. `src/logos.rs` `outline(mark, size)` 가 usvg 로 평탄화해 `font::Cmd` 로 반환(한 번 파싱 후 캐시), `src/font.rs` 는 `build()` 에서 `path_of(cmds, ..)` 를 뽑아 라벨·마크가 공유. `Shape::Mark` 를 `scene`·`paint`(해시·비용·빌드)·`svg`(공유 `path_data()`) 에 추가. `ChipStyle.logo` 와 `slots()` 가 `[pad][mark][gap][label][pad]` 를 잡고, 오프셋은 칩의 접선 방향이라 기울어진 칩에서도 같이 돌아간다. 세션 칩은 10px 마크, 서브에이전트는 0(가라앉는 16→12 에서는 뭉개진다, #53). 칩 폭은 마크+갭만큼 늘어난다. usvg 를 dev-dependency 에서 정식 의존성으로 올렸다. `--svg --demo` 로 렌더 확인함 |
| 81 | 2026-09-23 | 맨 아래 있는 라인은 스크롤이 잘려서 PR 이 보이지 않음 | ✅ | `clamp_scroll` 이 `content_height - layout.height` 를 상한으로 썬 게 원인 — `layout.height` 는 상태바를 포함한 창 전체라 끝까지 내려도 마지막 레인이 바 밑에 남는다. `max_scroll`/`visible_height` 로 도려내 `STATUS_H / zoom` 을 뺀다. 상태바는 고정 픽셀이라 줌을 줄일수록 디자인 단위로 커지고, 즐 0.88 아래에서 `BOTTOM_PAD`(34) 를 넘어 잘렸다(사용자 설정은 0.8). 테스트는 즐 0.5~3 × 세션 0~30 에서 맨 아래 레인이 상태바 위에 온전히 들어오는지를 본다 |
| 82 | 2026-09-23 | 머지된 PR은 보라색으로 해주라 | ✅ | `src/theme.rs` 에 `merged`/`on_merged` 추가: 흰 보드 `#8250DF`(GitHub 의 merged 보라), 주황 `#4B298C`+`#EFE6FF`(주황 위에서 따뜻한 덩어리로 읽히지 않게 깊게), dark `#A371F7`+보드의 검정. `scene` 의 `Look::Merged` 와 ⌘K 의 `Style::body()` 둘 다 `chip`→`merged` 로. 전에는 칩과 같은 검정이라 같은 레인 위 세션 칩과 구분이 안 됐다. `--svg --demo` 로 확인 |
| 83 | 2026-09-23 | 화면 크기에 따라 잘리기도 하는 시나리오가 있는 것 같아 · 크기와 무관하게 모두 보이게 | ✅ | 추측 대신 240×200~2400×1400 × 세션 1/3/9 를 훑는 테스트를 먼저 짜서 세 군데를 찾았다. (1) `session_anchor` 의 폴백이 왼쪽 경계만 보고 골라 칩을 오른쪽으로 내보냈다(240폭에서 FEATURE-REPORT) → 마지막에 화면 안으로 클램프하고 제 45° 위로 미끄러뜨린다. (2) 좁은 창에서는 칩이 뒤 스트라이프로 밀리고 스트라이프마다 레인이 `DIAG` 씩 더 내려가는데 `content_height` 는 레인만 재서 스크롤이 그 아래로 못 갔다 → `Layout::board_bottom()` 이 그려진 것에서 바닥을 잡고 `Board.content_bottom` 으로 다음 프레임 클램프에 쓴다. (3) ⌘K 카드(520 고정)와 도킹 패널(300/330)은 작은 창을 넘쳐 → `card_box()` 가 너비·상단·행 수를 창에 맞추고(선택 행은 항상 포함), `panel_chrome` 은 `Board.window` 로 좁힌다. 남은 한계: 340포 이하에서는 잘리지는 않지만 칩과 커넥터가 서로 겹친다(보드 문법상 최소 폭이 필요) |
